...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <aside
      className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#e8e4db]
    ...
    >
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-xl bg-[#3cc9e1] flex items-center justify-center text-white shadow-md">
              <Leaf size={24} />
            </div>
            <div>
              <h1 className="text-lg font-extrabold text-[#3cc9e1]">
                Roots of Love
              </h1>
              <p className="text-[10px] uppercase tracking-wider text-[#7d6e63]/60 font-bold">
                Donor Portal
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="lg:hidden text-[#7d6e63]"
          >
            <X size={24} />
          </button>
        </div>
        ...
        {/* Navigation */}
        <nav className="flex flex-col gap-1.5">
          <NavItem icon={LayoutDashboard} label="Overview" to="/donor-dashboard" />
          <NavItem icon={Calendar} label="My Adoptions" to="/my-adoptions" />
          <NavItem icon={HeartHandshake} label="Impact Stories" to="/impact-stories" />
          <NavItem icon={Users} label="Groups" to="/groups" />

          <div className="pt-6 mt-6 border-t border-[#e8e4db]">
            <NavItem icon={Settings} label="Account" to="/account" />
          </div>
        </nav>
      </div>
      ...
      <div className="bg-[#5a8c76]/5 p-4 rounded-2xl flex items-center gap-3 border border-[#5a8c76]/10">
        ...
        <div className="flex flex-col truncate">
          <p className="text-sm font-bold truncate">
            {user?.firstName || "Rajsi"} 👋
          </p>
          <p className="text-[10px] uppercase tracking-tighter text-[#3cc9e1] font-bold">
            Level 3 Donor
          </p>
        </div>
      </div>
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${active
          ? "bg-[#3cc9e1] text-white shadow-sm"
          : "text-[#7d6e63] hover:bg-[#5a8c76]/10 hover:text-[#3cc9e1]"}
      `}
    >
      <Icon size={20} />
